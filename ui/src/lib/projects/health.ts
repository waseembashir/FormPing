...
export function rollupFromHealth(health: UrlHealth[]): ProjectRollup {
  ...
  for (const h of health) {
    // Include STOPPED monitors' last known results: the result is kept until the
    // project is deleted, so the row must show it (and a real "checked" time)
    // rather than "—" and "never", which was simply untrue. `monitored` still
    // tracks LIVE monitors only, so the status word stays honest.
